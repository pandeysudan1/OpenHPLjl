...
# Closed-loop nonlinear hydro unit:
# Reservoir -> Headrace -> SurgeTank -> Penstock -> Turbine -> Tail
#                                      ^
#                                      |
#                                 DroopGovernor <- Generator frequency
#
# Electrical load increases by 10% at t = 5 s.
...
println("OpenHPLjl closed-loop FCR load-step example")
println("retcode = ", sol.retcode)
println("f(0) [Hz] = ", sol[gen.f][1])
println("f_final [Hz] = ", sol[gen.f][end])
println("u_final [pu] = ", sol[governor.u][end])
println("P_t_final [MW] = ", sol[turbine.P_t][end] / 1e6)
println("surge h_final [m] = ", sol[surge.h][end])

SciMLBase.successful_retcode(sol.retcode) || error("FCR load-step simulation failed")
