...
\title{Report 20 - DarcyConstant Model}
\author{Madhusudhan Pandey}
\date{20 September 2026}
\begin{document}\maketitle
\begin{abstract}This report documents the DarcyConstant model family in OpenHPLjl. Catalogue status: \textbf{baseline}.\end{abstract}
\section{Updated Engineering Context}Darcy-Weisbach loss with fixed friction factor. It belongs to the Friction models domain.
...
\section{Governing Mathematics}\[H_f=f_D\frac{L}{D}\frac{Q|Q|}{2gA^2}\]The production model may add states, constitutive relations, lookup maps, or spatial discretization.
...
\section{Julia / ModelingToolkit Implementation}Source: \texttt{\detokenize{src/FrictionModels/DarcyConstant.jl}}. The file is either an implemented component or an explicit Julia source specification.
...
\section{Engineering Interpretation}Use when a representative Darcy factor is known.
\section*{Conclusion}DarcyConstant now has an explicit source/report pair. Promotion to baseline requires validated equations, parameters, tests, and reference behaviour.
\end{document}