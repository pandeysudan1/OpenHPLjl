...
\title{Report 23 - SwameeJain Model}
\author{Madhusudhan Pandey}
\date{20 September 2026}
\begin{document}\maketitle
\begin{abstract}This report documents the SwameeJain model family in OpenHPLjl. Catalogue status: \textbf{baseline}.\end{abstract}
\section{Updated Engineering Context}Explicit turbulent Swamee-Jain correlation. It belongs to the Friction models domain.
...
\section{Governing Mathematics}\[f_D=\frac{0.25}{\log_{10}(\epsilon/(3.7D)+5.74/Re^{0.9})^2}\]The production model may add states, constitutive relations, lookup maps, or spatial discretization.
...
\section{Julia / ModelingToolkit Implementation}Source: \texttt{\detokenize{src/FrictionModels/SwameeJain.jl}}. The file is either an implemented component or an explicit Julia source specification.
...
\section{Engineering Interpretation}Cross-check against Haaland and Colebrook.
\section*{Conclusion}SwameeJain now has an explicit source/report pair. Promotion to baseline requires validated equations, parameters, tests, and reference behaviour.
\end{document}