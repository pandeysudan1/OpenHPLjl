...
\title{Report 52 - RoundRotorGenerator Model}
\author{Madhusudhan Pandey}
\date{20 September 2026}
\begin{document}\maketitle
\begin{abstract}This report documents the RoundRotorGenerator model family in OpenHPLjl. Catalogue status: \textbf{planned}.\end{abstract}
\section{Updated Engineering Context}Round-rotor synchronous-generator family. It belongs to the Generators domain.
...
\section{Governing Mathematics}\[X_d\approx X_q\]The production model may add states, constitutive relations, lookup maps, or spatial discretization.
...
\section{Julia / ModelingToolkit Implementation}Source: \texttt{\detokenize{src/Electrical/Generators/RoundRotorGenerator.jl}}. The file is either an implemented component or an explicit Julia source specification.
...
\section{Engineering Interpretation}Keep for interoperability with general power-system studies.
\section*{Conclusion}RoundRotorGenerator now has an explicit source/report pair. Promotion to baseline requires validated equations, parameters, tests, and reference behaviour.
\end{document}