...
\title{Report 22 - Haaland Model}
\author{Madhusudhan Pandey}
\date{20 September 2026}
\begin{document}\maketitle
\begin{abstract}This report documents the Haaland model family in OpenHPLjl. Catalogue status: \textbf{baseline}.\end{abstract}
\section{Updated Engineering Context}Explicit turbulent Haaland correlation. It belongs to the Friction models domain.
...
\section{Governing Mathematics}\[f_D=\left[-1.8\log_{10}\left((\epsilon/3.7D)^{1.11}+6.9/Re\right)\right]^{-2}\]The production model may add states, constitutive relations, lookup maps, or spatial discretization.
...
\section{Julia / ModelingToolkit Implementation}Source: \texttt{\detokenize{src/FrictionModels/Haaland.jl}}. The file is either an implemented component or an explicit Julia source specification.
...
\section{Engineering Interpretation}Use as a robust explicit turbulent option.
\section*{Conclusion}Haaland now has an explicit source/report pair. Promotion to baseline requires validated equations, parameters, tests, and reference behaviour.
\end{document}